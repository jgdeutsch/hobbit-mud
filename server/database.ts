...
export function initializeDatabase(): void {
  ...
  database.exec(`
    ...
    CREATE TABLE IF NOT EXISTS player_quests (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      player_id INTEGER NOT NULL,
      quest_giver_npc_id INTEGER NOT NULL,
      title TEXT NOT NULL,
      description TEXT,
    ...
    -- Quest steps (generated by Gemini upfront)
...
function applyMigrations(database: Database.Database): void {
  ...
  // Check if player_quests table exists
  const questTables = database.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='player_quests'").get();
  if (!questTables) {
    console.log('Applying migration: creating quest tables');
    database.exec(`
      CREATE TABLE IF NOT EXISTS player_quests (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        player_id INTEGER NOT NULL,
